    ...
    if request.method=="POST":
        try:
            ...
            for item in contents:
                ...
                playprofile_obj=BestvPlayprofile(ServiceType=item['servicetype'],
                    DeviceType=item['dev'],
                    ISP=item['isp'],
                    Area=item['area'],
                    Date=create_date,
...
def playtime(request):
    ...
    if request.method=="POST":
        try:
            ...
            for item in contents:
                create_date='%s-%s-%s'%(item['date'][0:4], item['date'][4:6], item['date'][6:8])
                playtime_obj=BestvPlaytime(ServiceType=item['servicetype'],
                    DeviceType=item['dev'],
                    ISP=item['isp'],
                    Area=item['area'],
                    ViewType=item['viewtype'],
...
def fbuffer(request):
    ...
    if request.method=="POST":
        try:
            ...
            for item in contents:
                create_date='%s-%s-%s'%(item['date'][0:4], item['date'][4:6], item['date'][6:8])
                fbuffer_obj=BestvFbuffer(ServiceType=item['servicetype'],
                    DeviceType=item['dev'],
                    ISP=item['isp'],
                    Area=item['area'],
                    ViewType=item['viewtype'],
...
def fluency(request):
    ...
    if request.method=="POST":
        try:
            ...
            for item in contents:
                create_date='%s-%s-%s'%(item['date'][0:4], item['date'][4:6], item['date'][6:8])
                fluency_obj=BestvFluency(ServiceType=item['servicetype'],
                    DeviceType=item['dev'],
                    ISP=item['isp'],
                    Area=item['area'],
                    ViewType=item['viewtype'],
