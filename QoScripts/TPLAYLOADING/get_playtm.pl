...
foreach my $key( sort {$fbuffer_map{$b} <=> $fbuffer_map{$a}} keys %fbuffer_map){
	...
	open OUT, ">> ${out}";
	printf OUT ("%s %d %s %s %d %d %d %d %d %.3f %.3f\n", ${date}, ${hour}, $srv_type, $key, $fbuffer_map{$key}, $stuck_map{$key}, $stuck_dbuffer_map{$key}, 
	...
	if($hour==24){
		...
		open DIST, ">> ./out/${srv_type}/total/playtm_of_$version_video[0]_$version_video[1]";
		printf DIST ("|%s|%s|%s|%d|%d|%d|%d|%d|%d|%.3f|%.3f|\n", ${date}, $srv_type, $version_video[0], $version_video[1], 
			$fbuffer_map{$key}, $stuck_map{$key}, $stuck_dbuffer_map{$key}, 
			$stucks_map{$key}, $stucks_dbuffers_map{$key}, , ($fbuffer_map{$key} - $stuck_map{$key})/$fbuffer_map{$key},
			($fbuffer_map{$key} - $stuck_dbuffer_map{$key})/$fbuffer_map{$key});
		close DIST;
	}
