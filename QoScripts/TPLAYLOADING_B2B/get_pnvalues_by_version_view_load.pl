...
if($suc_total>10){
	if(exists($video_hash{$file_id[5]}) && exists($load_hash{$file_id[6]})){
		open OUT, ">> ${out}";
		printf OUT ("%s 24 %s %s %d %d %d %d %d %d %d %d\n", ${date}, ${b2b_type}, $version, $file_id[5], $file_id[6], $sorted_time[$idx_25], $sorted_time[$idx_50], $sorted_time[$idx_75], $sorted_time[$idx_90], $sorted_time[$idx_95], $suc_total);
		close OUT;

		open DIST, ">> ./out/${b2b_type}/total/pnvalues_of_${version}_$video_hash{$file_id[5]}_$load_hash{$file_id[6]}";
